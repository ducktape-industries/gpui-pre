...
impl X11WindowStatePtr {
    ...
    pub fn set_bounds(&self, bounds: Bounds<i32>) -> anyhow::Result<()> {
        let (is_resize, content_size, scale_factor) = {
            ...
            // The event's origin is only right when the window manager sent
            // it (root coordinates); the server's own event under a
            // reparenting window manager is relative to the frame, and a
            // resize event's is often stale. Ask the server where the window
            // sits instead, falling back to the event for a pure move.
            ...
            match get_reply(
                ...
                Ok(root) => {
                    state.bounds.origin = gpui::point(
                        px(root.dst_x as f32 / state.scale_factor),
                        px(root.dst_y as f32 / state.scale_factor),
                    )
                }
                Err(_) if !is_resize => state.bounds.origin = bounds.origin,
                Err(_) => {}
            }
...
impl PlatformWindow for X11Window {
    ...
    fn set_bounds(&mut self, bounds: Bounds<Pixels>) {
        let (scale_factor, frame_extents) = {
            let state = self.0.state.borrow();
            (state.scale_factor, state.atoms._NET_FRAME_EXTENTS)
        };
        let bounds = bounds.to_device_pixels(scale_factor);
        // `bounds` reports where the client sits, but a reparenting window
        // manager puts its frame at the requested x/y (NorthWest gravity):
        // step back by the frame's left and top edges
        let (left, top) = get_reply(
            || "X11 GetProperty for _NET_FRAME_EXTENTS failed.",
        ...
        .ok()
        .and_then(|reply| {
            let edges: Vec<u32> = reply.value32()?.collect();
            (edges.len() == 4).then(|| (edges[0] as i32, edges[2] as i32))
        })
        .unwrap_or_default();
        let (x, y) = (bounds.origin.x.0 - left, bounds.origin.y.0 - top);
        let (width, height) = (bounds.size.width.0 as u32, bounds.size.height.0 as u32);
        check_reply(
            || {
                format!(
                    "X11 ConfigureWindow failed. x: {x}, y: {y}, width: {width}, height: {height}"
                )
            },
            self.0.xcb.configure_window(
                self.0.x_window,
