...
#[cold]
#[inline(never)]
fn custom_error(msg: &dyn Display) -> Error {
...
// String validation and reader buffering do not depend on the serde visitor.
// Preserve both reference lifetimes and the invalid-UTF8 byte fallback.
enum StringData<'de, 'a> {
    Borrowed(&'de str),
    Copied(&'a str),
    BorrowedBytes(&'de [u8], Utf8Error),
    CopiedBytes(&'a [u8], Utf8Error),
}

#[inline(never)]
fn read_string_data<'a, 'de, R: ReadSlice<'de>>(rd: &'a mut R, len: u32) -> Result<StringData<'de, 'a>, Error> {
    Ok(match read_bin_data(rd, len)? {
        Reference::Borrowed(buf) => match str::from_utf8(buf) {
            Ok(value) => StringData::Borrowed(value),
            Err(error) => StringData::BorrowedBytes(buf, error),
        },
        Reference::Copied(buf) => match str::from_utf8(buf) {
            Ok(value) => StringData::Copied(value),
            Err(error) => StringData::CopiedBytes(buf, error),
        },
    })
}
...
{
    match read_string_data(rd, len)? {
        StringData::Borrowed(value) => visitor.visit_borrowed_str(value),
        StringData::Copied(value) => visitor.visit_str(value),
        StringData::BorrowedBytes(buf, error) => visitor
            .visit_borrowed_bytes::<Error>(buf)
            .map_err(|_| Error::Utf8Error(error)),
        StringData::CopiedBytes(buf, error) => visitor
            .visit_bytes::<Error>(buf)
            .map_err(|_| Error::Utf8Error(error)),
    }
}
...
// Parse marker payloads once per reader type rather than once per serde visitor.
// Variant widths are retained because visitors may distinguish numeric widths.
enum Number {
    Unit,
    Bool(bool),
    U8(u8),
...
#[inline(never)]
fn read_num<'de, R: ReadSlice<'de>>(rd: &mut R, marker: Marker) -> Result<Number, Error> {
    Ok(match marker {
        Marker::Null => Number::Unit,
        Marker::True | Marker::False => Number::Bool(marker == Marker::True),
        Marker::FixPos(val) => Number::U8(val),
        Marker::FixNeg(val) => Number::I8(val),
        Marker::U8 => Number::U8(rd.read_data_u8()?),
        Marker::U16 => Number::U16(rd.read_data_u16()?),
        Marker::U32 => Number::U32(rd.read_data_u32()?),
        Marker::U64 => Number::U64(rd.read_data_u64()?),
        Marker::I8 => Number::I8(rd.read_data_i8()?),
        Marker::I16 => Number::I16(rd.read_data_i16()?),
        Marker::I32 => Number::I32(rd.read_data_i32()?),
        Marker::I64 => Number::I64(rd.read_data_i64()?),
        Marker::F32 => Number::F32(rd.read_data_f32()?),
        Marker::F64 => Number::F64(rd.read_data_f64()?),
        other_marker => return Err(Error::TypeMismatch(other_marker)),
    })
}

fn visit_num<'de, V: Visitor<'de>>(number: Number, visitor: V) -> Result<V::Value, Error> {
    match number {
        Number::Unit => visitor.visit_unit(),
        Number::Bool(value) => visitor.visit_bool(value),
...
fn any_num<'de, R: ReadSlice<'de>, V: Visitor<'de>>(rd: &mut R, visitor: V, marker: Marker) -> Result<V::Value, Error> {
    visit_num(read_num(rd, marker)?, visitor)
}
...
#[inline(never)]
fn read_head<'de, R: ReadSlice<'de>>(rd: &mut R, marker: Marker) -> Result<Head, Error> {
    Ok(match marker {
        ...
        Marker::F32 | Marker::F64 => Head::Number(read_num(rd, marker)?),
        Marker::FixStr(len) => Head::Str(len.into()),
        Marker::Str8 => Head::Str(read_u8(rd)?.into()),
        Marker::Str16 => Head::Str(read_u16(rd)?.into()),
        Marker::Str32 => Head::Str(read_u32(rd)?),
        Marker::FixArray(len) => Head::Array(len.into()),
        Marker::Array16 => Head::Array(read_u16(rd)?.into()),
        Marker::Array32 => Head::Array(read_u32(rd)?),
        Marker::FixMap(len) => Head::Map(len.into()),
        Marker::Map16 => Head::Map(read_u16(rd)?.into()),
        Marker::Map32 => Head::Map(read_u32(rd)?),
        Marker::Bin8 => Head::Bin(read_u8(rd)?.into()),
        Marker::Bin16 => Head::Bin(read_u16(rd)?.into()),
        Marker::Bin32 => Head::Bin(read_u32(rd)?),
        Marker::FixExt1 | Marker::FixExt2 | Marker::FixExt4 |
        Marker::FixExt8 | Marker::FixExt16 |
        Marker::Ext8 | Marker::Ext16 | Marker::Ext32 => Head::Ext(ext_len(rd, marker)?),
        Marker::Reserved => return Err(Error::TypeMismatch(Marker::Reserved)),
    })
}

impl<'de, R: ReadSlice<'de>, C: SerializerConfig> Deserializer<R, C> {
    // Keep upstream's fallback even when reading a map length fails: only a
    // successfully parsed singleton map clears the saved marker.
    #[inline(never)]
    fn read_enum_head(&mut self) -> Result<bool, Error> {
    ...
    fn any_inner<V: Visitor<'de>>(&mut self, visitor: V, allow_bytes: bool) -> Result<V::Value, Error> {
        self.value_inner::<V, 63>(visitor, allow_bytes)
    }

    // Shape bits: number=1, string=2, array=4, map=8, binary=16, extension=32.
    // The full reader retains every shape; typed calls compile only hinted shapes.
    fn value_inner<V: Visitor<'de>, const SHAPES: u8>(&mut self, visitor: V, allow_bytes: bool) -> Result<V::Value, Error> {
        let marker = self.take_or_read_marker()?;
        match read_head(&mut self.rd, marker)? {
            Head::Number(number) if SHAPES & 1 != 0 => visit_num(number, visitor),
            Head::Str(len) if SHAPES & 2 != 0 => read_str_data(&mut self.rd, len, visitor),
            Head::Array(len) if SHAPES & 4 != 0 => {
...
impl<'de, R: ReadSlice<'de>, C: SerializerConfig> serde::Deserializer<'de> for &mut Deserializer<R, C> {
    ...
    fn deserialize_seq<V>(self, visitor: V) -> Result<V::Value, Self::Error> where V: Visitor<'de> {
        #[cfg(feature = "typed")]
        { self.value_inner::<V, 20>(visitor, false) }
    ...
    fn deserialize_struct<V>(self, _: &'static str, _: &'static [&'static str], visitor: V) -> Result<V::Value, Self::Error> where V: Visitor<'de> {
        #[cfg(feature = "typed")]
        { self.value_inner::<V, 12>(visitor, false) }
        #[cfg(not(feature = "typed"))]
        { self.any_inner(visitor, false) }
    }
    ...
    fn deserialize_bytes<V>(self, visitor: V) -> Result<V::Value, Self::Error> where V: Visitor<'de> {
        #[cfg(feature = "typed")]
        { self.value_inner::<V, 22>(visitor, true) }
        #[cfg(not(feature = "typed"))]
        { self.any_inner(visitor, true) }
    }
    ...
    fn deserialize_unit<V>(self, visitor: V) -> Result<V::Value, Self::Error> where V: Visitor<'de> {
        #[cfg(feature = "typed")]
        { self.value_inner::<V, 1>(visitor, true) }
        #[cfg(not(feature = "typed"))]
        { self.any_inner(visitor, true) }
    }
    ...
    fn deserialize_identifier<V>(self, visitor: V) -> Result<V::Value, Self::Error> where V: Visitor<'de> {
        #[cfg(feature = "typed")]
        { self.value_inner::<V, 19>(visitor, true) }
        #[cfg(not(feature = "typed"))]
        { self.any_inner(visitor, true) }
    }
    ...
    fn deserialize_map<V>(self, visitor: V) -> Result<V::Value, Self::Error> where V: Visitor<'de> {
        #[cfg(feature = "typed")]
        { self.value_inner::<V, 8>(visitor, true) }
        #[cfg(not(feature = "typed"))]
        { self.any_inner(visitor, true) }
    }
    ...
    fn deserialize_str<V>(self, visitor: V) -> Result<V::Value, Self::Error> where V: Visitor<'de> {
        #[cfg(feature = "typed")]
        { self.value_inner::<V, 18>(visitor, true) }
        #[cfg(not(feature = "typed"))]
        { self.any_inner(visitor, true) }
    }
